  ...
  static Future<bool> addScreen(String screenName) async {
    try {
      ...
      if (screens.contains(screenName)) {
        ...
        print("ERROR ReviewService addScreen: no estaba contemplada esta pantalla");
        return false;
      }
    ...
    try {
      ...
      if (lastRequestIsoDate != null && lastRequestIsoDate.isNotEmpty) {
        ...
        if (lastRequestDate.isAfter(minDate)) {
          print("ReviewService mustRequest: no ha pasado el minimo desde la ultima request $lastRequestIsoDate");
          return false;
        } else {
          return true;
        }
    ...
    try {
      ...
      if (mustRequest) {
        if (await inAppReview.isAvailable()) {
          print("Ejectuando inappReview");
          await inAppReview.requestReview();
          prefs.setString("reviewLastRequest", DateTime.now().toIso8601String());
        } else {
          print("ReviewService checkAndRequestReview: inAppReview no está disponible");
        }
      } else {
